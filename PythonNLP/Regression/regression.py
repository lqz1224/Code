...
def regression1():
    xArr, yArr = loadDataSet(path+"/Regression/data/data.txt")
    xMat = mat(xArr)
    yMat = mat(yArr)
...
def abaloneTest():
    ...
    '''
    # 加载数据
    abX, abY = loadDataSet(path+"/Regression/data/abalone.txt")
    # 使用不同的核进行预测
...
# test for stageWise
def regression4():
    xArr, yArr = loadDataSet(path+"/Regression/data/abalone.txt")
